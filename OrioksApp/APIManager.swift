...
struct APIManager {
    ...
    static func revokeToken(token: String, completion: @escaping (Result<Void, Error>) -> Void) {
        guard let url = URL(string: "\(baseURL)/api/v1/student/tokens/\(token)") else {
            completion(.failure(APIError.invalidURL))
            return
        }
        var request = URLRequest(url: url)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let error = error {
                print("Ошибка аннулирования токена: \(error)")
                completion(.failure(error))
                return
            }
            guard let httpResponse = response as? HTTPURLResponse,
                ...
                print("Ошибка HTTP при аннулировании токена: \(statusCode)")
                completion(.failure(APIError.httpError(code: statusCode)))
                return
            }
            completion(.success(()))
        }.resume()
